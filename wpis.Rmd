---
title: "Jakie imiona są do siebie podobne + wykres strunowy"
...
- **odległość Damerau-Levenshteina** - podobne jak odległość Levenshteina, ale wprowadza działanie polegające na zamianie miejscami sąsiadujących znaków.
- **najdłuższy wspólny podłańcuch** (ang. longest common substring) - najdłuższy ciąg elementem leżących obok siebie w obu ciągach. Odległość to liczba niesparowanych znaków.
...
Na podstawie wyżej opisanych algorytmów można stwierdzić, że im mniejsza wartość danej odległości tym ciągi będą do siebie bardziej podobne. Pakiet `stringdist` zawiera implementację jeszcze kilku miar, niemniej w tym poście skupiłem się na tych najbardziej popularnych.
...
Do przedstawienia uzyskanych relacji najlepszy będzie wykres strunowy (ang. chord diagram). W pakiecie `chorddiag` znajduje się eRowa implementacja tego wykresu pochodząca z biblioteki [d3.js](https://d3js.org/). Większa wersja tego wykresu znajduje się [tutaj](html/dl.html).
...
Wyniki także przedstawione są na wykresie strunowym ([większa wersja](html/lcs.html)).
...
Uzyskane wyniki przedstawione na wykresie strunowym ([większa wersja](html/jd.html)).
